...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    for(let dev = 0; dev < programmers.length; dev ++){
      
     if( dev + 1 == idFeature){
       return programmers[dev].name}
     }
    // Leave this for last!
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setIdFeature(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
